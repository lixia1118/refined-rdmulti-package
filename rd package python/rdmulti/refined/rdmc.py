...
def rdmc(Y, X, C, fuzzy=None, derivvec=None, pooled_opt=None, verbose=False,
    ...
    - output_path: path to save the results to a file.
    - plot_path: path to save the plots to a file.
    ...
    try:
        ...
        raise Exception('C has to be numeric')
    ...
    if hmat is not None:
        if np.isscalar(hmat):
            hmat = np.full((cnum, 2), hmat)
        elif len(hmat) == cnum:
            hmat = np.column_stack((hmat, hmat))
        elif len(hmat) == 2:
            hmat = np.tile(hmat, (cnum, 1))
    else:
        hmat = np.repeat(None, cnum)

    if bmat is not None:
        if np.isscalar(bmat):
            hmat = np.full((cnum, 2), bmat)
        elif len(bmat) == cnum:
            bmat = np.column_stack((bmat, bmat))
        elif len(bmat) == 2:
            bmat = np.tile(bmat, (cnum, 1))
    else:
        bmat = np.repeat(None, cnum)
    ...
    if pooled_opt is None:
        pooled_opt = ''
    else:
        pooled_opt = "," + pooled_opt
    ...
    if plot == True:
        ...
        for k in range(cnum):
            if k == 0:
                ax1.plot(clist[k], Coefs[0, k], 'o', c='blue', label='Estimate')
                if conventional == False:
                    ax1.plot(np.repeat(clist[k], 2), CI[:, k], c='blue', label=f'{level}% CI')
                else:
                    ax1.plot(np.repeat(clist[k], 2), CI_cl[:, k], c='blue', label=f'{level}% CI')
            else:
                ax1.plot(clist[k], Coefs[0, k], 'o', c='blue')
                if conventional == False:
                    ax1.plot(np.repeat(clist[k], 2), CI[:, k], c='blue')
                else:
                    ax1.plot(np.repeat(clist[k], 2), CI_cl[:, k], c='blue')

        ax1.axhline(y=Coefs[0, cnum + 1], color='gray', label='Pooled estimate')
        ax1.axhline(y=Coefs[0, cnum], color='red', label='Weighted estimate')
        ax1.axhline(y=0, color='black', linestyle='dotted')
        ax1.fill_between(xlim, CI[0, cnum], CI[1, cnum], color='red', alpha=.1)
        ax1.fill_between(xlim, CI[0, cnum + 1], CI[1, cnum + 1], color='gray', alpha=.2)
        ax1.set_xlabel("Cutoff")
        ax1.set_ylabel("Treatment Effect")
        ax1.legend()

        # 第二个子图: 权重
        ax2.bar(np.array(clist, dtype=str), height=W[0], color='gray')
    ...
    if output_path is not None:
        ...
        if file_ext == 'txt':
            with open(output_path, 'w') as f:
                ...
                if not all(x is None for x in derivvec):
                    f.write(f"derivvec: {derivvec}\n")
                if not all(x is None for x in pvec):
                    f.write(f"pvec: {pvec}\n")
                if not all(x is None for x in qvec):
                    f.write(f"qvec: {qvec}\n")
                if not all(x is None for x in rhovec):
                    f.write(f"rhovec: {rhovec}\n")
                if not all(x == 'tri' for x in kernelvec):
                    f.write(f"kernelvec: {kernelvec}\n")
                if not all(x == 'mserd' for x in bwselectvec):
                    f.write(f"bwselectvec: {bwselectvec}\n")
                if not all(x == 'nn' for x in vcevec):
                    f.write(f"vcevec: {vcevec}\n")
                if level != 95:
                    f.write(f"level: {level}\n")
                if conventional:
                    f.write(f"conventional: {conventional}\n")
                if cluster is not None:
                    f.write(f"cluster: used\n")
                if covs_mat is not None:
                    f.write(f"covs_mat: {covs_mat.shape[1]} covariates included\n")
                if covs_list is not None:
                    f.write(f"covs_list: {covs_list}\n")
                if not all(x == True for x in covs_dropvec):
                    f.write(f"covs_dropvec: {covs_dropvec}\n")
                if not all(x == 3 for x in nnmatchvec):
                    f.write(f"nnmatchvec: {nnmatchvec}\n")
                if not all(x == 1 for x in scaleparvec):
                    f.write(f"scaleparvec: {scaleparvec}\n")
                if not all(x == 1 for x in scaleregulvec):
                    f.write(f"scaleregulvec: {scaleregulvec}\n")
                if not all(x == 'adjust' for x in masspointsvec):
                    f.write(f"masspointsvec: {masspointsvec}\n")
                if not all(x == True for x in bwrestrictvec):
                    f.write(f"bwrestrictvec: {bwrestrictvec}\n")
                if not all(x == False for x in stdvarsvec):
                    f.write(f"stdvarsvec: {stdvarsvec}\n")
                f.write('\n')

                f.write('=' * 90 + '\n')
                # 分析结果
                if conventional == False:
                    f.write('Cutoff-specific RD estimation with robust bias-corrected inference\n')
                else:
                    f.write('Cutoff-specific RD estimation with conventional inference\n')

                f.write('=' * 90 + '\n')
                f.write(f"{'Cutoff':11}{'Coef.':8}{'P-value':16}{'95% CI':16}{'hl':9}{'hr':9}{'Nh':5}{'Weight':5}\n")
                f.write('=' * 90 + '\n')

                # 写入每个cutoff的结果
                for k in range(cnum):
                    f.write(f"{clist[k]:11.3f}{Coefs[0, k]:8.3f}{Pv[0, k]:9.3f}{CI[0, k]:10.3f}{CI[1, k]:10.3f}"
                            f"{H[0, k]:9.3f}{H[1, k]:9.3f}{int(Nh[0, k] + Nh[1, k]):8d}{W[0, k]:5.3f}\n")

                # 写入加权和汇总结果
                f.write('=' * 90 + '\n')
                f.write(f"{'Weighted':11}{Coefs[0, cnum]:8.3f}{Pv[0, cnum]:9.3f}{CI[0, cnum]:10.3f}"
                        f"{CI[1, cnum]:10.3f}{'  .':9}{'  .':9}{int(Nh[0, cnum] + Nh[1, cnum]):8d}{'  .':5}\n")
                f.write(f"{'Pooled':11}{Coefs[0, cnum + 1]:8.3f}{Pv[0, cnum + 1]:9.3f}{CI[0, cnum + 1]:10.3f}"
                        f"{CI[1, cnum + 1]:10.3f}{H[0, cnum + 1]:9.3f}{H[1, cnum + 1]:9.3f}"
                        f"{int(Nh[0, cnum + 1] + Nh[1, cnum + 1]):8d}{'  .':5}\n")
                f.write('=' * 90 + '\n')

        else:
            raise ValueError(f"Unsupported file format: {file_ext}. Please use '.txt'")
