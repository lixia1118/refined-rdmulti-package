...
def rdmcplot(Y, X, C, nbinsmat=None, binselectvec=None, scalevec=None,
    ...
    Parameters:
        ...
        marker_size: Size of the data points.
    ...
    for c in clist:
        ...
        try:
            ...
            if ci is not None:
                ci_l = aux.vars_bins.iloc[:, 7].values
                ci_r = aux.vars_bins.iloc[:, 8].values
                ci_l = ci_l[~np.isnan(ci_l)]
                ci_r = ci_r[~np.isnan(ci_r)]

                CI_l[:len(ci_l), count] = ci_l
                CI_r[:len(ci_r), count] = ci_r
        finally:
            count += 1
    ...
    colorlist = ['darkblue', 'darkred', 'darkgreen', 'darkorange', 'gray50', 'khaki4', 'brown3', 'blue',
                 'darkgoldenrod4', 'cyan4']
    ...
    if xlabel:
        ...
        plt.xlabel('Running variable', fontsize=font_size)
    if ylabel:
        plt.ylabel(ylabel, fontsize=font_size)
    else:
        plt.ylabel('Outcome', fontsize=font_size)
    ...
    if legend_labels is None:
        legend_labels = [f'Cutoff {i + 1}' for i in range(cnum)]
    ...
    if ci is not None:
        for c in range(cnum):
            yerr_low = Ymean.iloc[:, c].values - CI_l.iloc[:, c].values
            yerr_high = CI_r.iloc[:, c].values - Ymean.iloc[:, c].values

            yerr_low = np.maximum(0, yerr_low)
            yerr_high = np.maximum(0, yerr_high)
    ...
    if not noxline:
        for c in range(cnum):
            rdmc_plot.axvline(x=clist[c],
                              color=col_xline[c],
                              linestyle=lty_xline[c],
                              linewidth=line_width)

    # 添加图例
    if not nobins and show_legend:
